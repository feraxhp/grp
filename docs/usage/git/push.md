...
~~~bash
grp push [OPTIONS] [pconf] [remote] [branch]
~~~

grp tries to imitate as much as posible the behavior of `git push` 
...
> splicitly call `grp push other`

### Flags

- `-A, --all`: Push all branches
- `-B, --branches`: Push all branches
- `-T, --tags`: Push all tags
- `-t, --tag[=<tag>]`: Push te given tag
